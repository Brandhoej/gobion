...
func (interpreter *scfgInterpreter) Branch(
	path *GoPath, flow *scfg.Graph[ast.Stmt, ast.Expr], block int, condition *z3.AST,
) *GoPath {
	scopeID, _ := flow.ScopeWith(block)
	if scope, exists := interpreter.scopes[scopeID]; exists {
		return path.BranchInto(condition, scope)
	}

	// TODO: scopeID should ensure all parent scopes are created and maybe 
	// interpreter.scopes should be use to branch into a specific scope.
	parentID, _ := flow.Parent(scopeID)
	parent, _ := interpreter.scopes[parentID]
	scope := parent.Child(scopeID)
	interpreter.scopes[scopeID] = scope
...
func (interpreter *scfgInterpreter) Jump(
	...
	if branch := interpreter.Branch(path, flow, destination, equality); branch != nil {
